...
const reduced = arrayOfWords.reduce((acc, cv) => {
  const inner = () => {
    let startCounter = 0;
    let endCounter = cv.length;
    let startNumber = 0;
    let endNumber = 0;

    const searchFromStart = () => {
      if (startCounter === cv.length + 1) {
        return;
      }
      if (parseInt(cv[startCounter])) {
        startNumber = cv[startCounter];
        return;
      } else {
        startCounter++;
        searchFromStart();
    ...
    const searchFromEnd = () => {
      ...
      if (parseInt(cv[endCounter])) {
        endNumber = parseInt(cv[endCounter]);
        return;
      } else {
        endCounter--;
        searchFromEnd();
    ...
    const toStringStart = startNumber.toString();
    const toStringEnd = endNumber.toString();
    const final = `${toStringStart}${toStringEnd}`;
    const finalParsed = parseInt(final);
